...
class StableDiffusionXLParticlePipeline(
    DiffusionPipeline,
    FromSingleFileMixin,
    StableDiffusionXLLoraLoaderMixin,
    TextualInversionLoaderMixin,
    ...
    def prepare_extra_step_kwargs(self, generator, eta):
        ...
        # eta (ÃƒÅ½Ã‚Â·) is only used with the DDIMScheduler, it will be ignored for other schedulers.
        # eta corresponds to ÃƒÅ½Ã‚Â· in DDIM paper: https://arxiv.org/abs/2010.02502
    ...
    def fuse_qkv_projections(self, unet: bool = True, vae: bool = True):
        ...
        This API is ÃƒÂ°Ã…Â¸Ã‚Â§Ã‚Âª experimental.
    ...
    def __call__(
        ...
        coeff = 0.,
        svgd = False,
        direction_guidance = None,
        direction_guidance_save = 0,
        **kwargs,
    ):
        r"""
        ...
        Args:
            ...
            eta (`float`, *optional*, defaults to 0.0):
                Corresponds to parameter eta (ÃƒÅ½Ã‚Â·) in the DDIM paper: https://arxiv.org/abs/2010.02502. Only applies to
            ...
            guidance_rescale (`float`, *optional*, defaults to 0.0):
                ...
                Flawed](https://arxiv.org/pdf/2305.08891.pdf) `guidance_scale` is defined as `ÃƒÂÃ¢â‚¬Â ` in equation 16. of
            ...
            negative_target_size (`Tuple[int]`, *optional*, defaults to (1024, 1024)):
                ...
                [https://huggingface.co/papers/2307.01952](https://huggingface.co/papers/2307.01952). For more
                information, refer to this issue thread: https://github.com/huggingface/diffusers/issues/4208.
            direction_guidance (`Callable`, *optional*):
                Performs direction guidance after the first step.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_classifier_free_guidance:
                    ...
                    if svgd:
                        scores = noise_pred_text + self.guidance_scale * (noise_pred_text - noise_pred_uncond)
                        ...
                        #distance = torch.nn.functional.normalize(distance, dim=1)

                        num_images = latents_vec.shape[0]
                        h_t = (distance.median(dim=1, keepdim=True)[0]) **2 / np.log(num_images - 1)
                        #print("\n latentsvec", latents_vec, "\n lateten_min, max:", torch.min(latents_vec, dim =1) , torch.max(latents_vec, dim =1),"\n diff :", distance, "\n diff :", distance, "\n ht:", h_t, "\n distance_median", (distance.median(dim=1, keepdim=True)[0]))
                        #"ht shape:", h_t.shape, 
                        weights = torch.exp(- (distance ** power / h_t))
                ...
                # perform guidance
                if self.do_classifier_free_guidance:
                    noise_pred = noise_pred_uncond + self.guidance_scale * (noise_pred_text - noise_pred_uncond) - grad_phi
                ...
                # prerform direction_guidance
                if direction_guidance is not None:
                    if i in [1,2,3,4,5]:
                        cond_grad = reference_direction(latents, direction_guidance, scale=self.guidance_rescale)
                        latents = latents.detach() + cond_grad

                if direction_guidance_save:
                    saved_latents.append(latents.detach())
        ...
        if not return_dict:
            return (image,saved_latents)

        return StableDiffusionXLPipelineOutput(images=image), saved_latents
